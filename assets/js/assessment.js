...
( function ( $ ) {
	...
	------------------------------------------------------- */
	var $modal, $overlay, $panel, $body,
	    $progressContainer, $progressBar, $progressLabel,
	    $screens, $infoForm, $infoError, $startBtn,
	    $categoryLabel, $questionCounter, $questionText,
	    $answerOptions, $questionError,
	    $backBtn, $nextBtn,
	    $resultsContent;
	...
	function showScreen( name ) {
		...
		var $target;
		switch ( name ) {
			case 'info':      $target = $( '#ca-screen-info' );      break;
			case 'questions': $target = $( '#ca-screen-questions' ); break;
			case 'results':   $target = $( '#ca-screen-results' );   break;
			case 'loading':   $target = $( '#ca-screen-loading' );   break;
		}
		...
		$.post( CA_Config.ajax_url, data )
			.done( function ( response ) {
				if ( response.success ) {
					...
					showError( $infoError, response.data.message || CA_Config.labels.error_generic );
				}
			} )
			.fail( function () {
	...
	function renderQuestion( data, index ) {
		...
		if ( selectedVal ) {
			var $opt = $modal.find( '.ca-answer-option[data-value="' + selectedVal + '"]' );
		...
		$.post( CA_Config.ajax_url, data )
			.done( function ( response ) {
				if ( response.success ) {
					...
					showError( $questionError, response.data.message || CA_Config.labels.error_generic );
				}
			} )
			.fail( function () {
	...
	function renderResults( data ) {
		...
		var initials = ( user.first_name.charAt( 0 ) + user.last_name.charAt( 0 ) ).toUpperCase();

		// Category cards
		var catHtml = '';
		cats.forEach( function ( cat ) {
			var pct = Math.round( ( cat.average / 5 ) * 100 );
			catHtml += '<div class="ca-cat-card">' +
				'<div class="ca-cat-card-header">' +
					'<span class="ca-cat-name">' + escHtml( cat.name ) + '</span>' +
					'<span class="ca-cat-score-badge">' +
						'<span class="ca-cat-score-num">' + parseFloat( cat.average ).toFixed( 2 ) + '</span>' +
						'<span class="ca-cat-score-max">/ 5</span>' +
					'</span>' +
				'</div>' +
				'<div class="ca-cat-bar-track"><div class="ca-cat-bar-fill" style="width:0%" data-width="' + pct + '%"></div></div>' +
				'<p class="ca-cat-summary">' + escHtml( cat.summary ) + '</p>' +
			'</div>';
		} );

		var html =
			'<div class="ca-results-hero">' +
				'<p class="ca-results-hero-name">' + escHtml( user.first_name + ' ' + user.last_name ) + ' — ' + escHtml( user.job_title ) + '</p>' +
				'<h2 class="ca-results-profile">' + escHtml( profile ) + '</h2>' +
				'<div class="ca-results-scores-row">' +
					'<div class="ca-results-score-item">' +
						'<span class="ca-results-score-num">' + total + '<sup>/' + maxScore + '</sup></span>' +
						'<span class="ca-results-score-label">Total Score</span>' +
					'</div>' +
					'<div class="ca-results-score-item">' +
						'<span class="ca-results-score-num">' + avg + '<sup>/5</sup></span>' +
			...
			'<div class="ca-results-body">' +
				'<div class="ca-results-user-card">' +
					'<div class="ca-results-user-avatar">' + escHtml( initials ) + '</div>' +
					'<div class="ca-results-user-info">' +
						'<p class="ca-results-user-name">' + escHtml( user.first_name + ' ' + user.last_name ) + '</p>' +
						'<p class="ca-results-user-detail">' + escHtml( user.email ) + ' &nbsp;·&nbsp; ' + escHtml( user.phone ) + '</p>' +
					'</div>' +
				'</div>' +
